  ...
  try {
    ...
    const geminiResponse = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-pro:generateContent?key=${geminiApiKey}`, {
      ...
      body: JSON.stringify({
        contents: [{
          parts: [
            { text: "Analyser billedet og identificer præcist hvilke specifikke genstande du kan se. \n\nVigtige regler:\n1. Vær MEGET specifik med navnene - brug ikke generelle termer\n2. Hvis du ser et batteri, specificer typen: \"bilbatteri\", \"telefon batteri\", \"AA batteri\" osv.\n3. Hvis du ser elektronik, specificer hvad det er: \"mobiltelefon\", \"computer\", \"radio\" osv.\n4. Hvis du ser emballage, specificer: \"plastikflaske\", \"pizzaboks\", \"konservesdåse\" osv.\n\nSlet eksempler på hvad jeg vil have:\n- I stedet for \"farligt materiale\" → \"bilbatteri\"\n- I stedet for \"elektronik\" → \"mobiltelefon\"\n- I stedet for \"plastik\" → \"plastikflaske\"\n- I stedet for \"metal\" → \"aluminiumsdåse\"\n\nFor hver specifik genstand:\n- description: Det præcise navn (f.eks. \"bilbatteri\", \"pizzaboks\", \"plastikflaske\")\n- materiale: Hovedmaterialet (pap, plastik, glas, metal, elektronik, farligt, organisk, tekstil)\n- score: Din sikkerhed (0.1-1.0)\n\nReturner kun JSON: {\"komponenter\":[{\"description\":\"præcist_navn\",\"materiale\":\"kategori\",\"score\":0.9}]}" },
            { 
              inline_data: {
